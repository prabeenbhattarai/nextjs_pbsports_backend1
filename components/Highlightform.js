...
const [title,setTitle] = useState(existingTitle || '');
    ...
    const [goToHighlight,setGoToHighlight] = useState(false);
    ...
    async function saveHighlight(ev){
        ev.preventDefault();
        const data={title,description,url,images,categories};


        if(_id){
            //update
            await axios.put('/api/highlight', {...data,_id});


        } else{
            //create
 await axios.post('/api/highlight', data);
        }
        
 setGoToHighlight(true);
    }
    if (goToHighlight){
  ...
  <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" onSubmit={saveHighlight}>
...
{!!images?.length && images.map(link =>(
    <div key={link} className=" h-24 ">
       <Image src={link} alt="" width={90} height={60}className="rounded-lg"/>
        </div>
    ))}
    </ReactSortable>
    {isUploading && (
